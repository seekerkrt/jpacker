...
void test_internal_edges_propagate_roots_to_remote_subtree() {
    ...
    try {
        ...
        malformed_projection_failed =
            std::string(error.what()).find("AUR package metadata constraint projection failed: " + one_way_remote) != std::string::npos;
    }
    expect(
        malformed_projection_failed,
...
void test_remote_provider_local_name_collision_fails_closed() {
    ...
    expect(
        repository_plan.failures().size() == 1 &&
            ...
            repository_plan.failures()[0].candidates[0].remote_provider == repository_provider,
    ...
    expect(
        aur_plan.failures().size() == 1 &&
            ...
            aur_plan.failures()[0].candidates[0].remote_provider.has_value(),
