...
[ "$#" -eq 101 ] ||
    fail "Make focused alias inventory is $#, expected 101"
...
[ "$(LC_ALL=C sort -u "$make_aliases" | wc -l)" -eq 101 ] ||
...
[ "$(wc -l < "$cmake_aliases")" -eq 101 ] ||
    fail "CMake focused target inventory is $(wc -l < "$cmake_aliases"), expected 101"
...
printf '%s\n' \
    'build-authority-closure-test: Make aliases=101, CMake targets=101, missing=0, unexpected=0'
