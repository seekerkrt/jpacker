...
[ "$#" -eq 100 ] ||
    fail "Make focused alias inventory is $#, expected 100"
...
[ "$(LC_ALL=C sort -u "$make_aliases" | wc -l)" -eq 100 ] ||
...
[ "$(wc -l < "$cmake_aliases")" -eq 100 ] ||
    fail "CMake focused target inventory is $(wc -l < "$cmake_aliases"), expected 100"
...
printf '%s\n' \
    'build-authority-closure-test: Make aliases=100, CMake targets=100, missing=0, unexpected=0'
