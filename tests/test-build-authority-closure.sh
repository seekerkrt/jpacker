...
[ "$#" -eq 98 ] ||
    fail "Make focused alias inventory is $#, expected 98"
...
[ "$(LC_ALL=C sort -u "$make_aliases" | wc -l)" -eq 98 ] ||
...
[ "$(wc -l < "$cmake_aliases")" -eq 98 ] ||
    fail "CMake focused target inventory is $(wc -l < "$cmake_aliases"), expected 98"
...
printf '%s\n' \
    'build-authority-closure-test: Make aliases=98, CMake targets=98, missing=0, unexpected=0'
