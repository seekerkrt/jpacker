...
[ "$#" -eq 99 ] ||
    fail "Make focused alias inventory is $#, expected 99"
...
[ "$(LC_ALL=C sort -u "$make_aliases" | wc -l)" -eq 99 ] ||
...
[ "$(wc -l < "$cmake_aliases")" -eq 99 ] ||
    fail "CMake focused target inventory is $(wc -l < "$cmake_aliases"), expected 99"
...
printf '%s\n' \
    'build-authority-closure-test: Make aliases=99, CMake targets=99, missing=0, unexpected=0'
