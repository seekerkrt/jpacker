...
assert_no_sudo_commands() {
    if grep -E '^sudo ' "$command_log" >/dev/null; then
        echo "source-only route reached a sudo transaction" >&2
        cat "$command_log" >&2
        exit 1
    fi
}
...
assert_only_sudo_command() {
    ...
    if [ "$sudo_count" -ne 1 ]; then
        echo "unexpected additional sudo transaction" >&2
        cat "$command_log" >&2
        exit 1
    fi
}
...
assert_single_makepkg_needed() {
    makepkg_count=$(grep -c '^makepkg ' "$command_log" || true)
    if [ "$makepkg_count" -ne 1 ]; then
        echo "expected exactly one makepkg command" >&2
        cat "$command_log" >&2
        exit 1
    fi
    needed_count=$(grep '^makepkg ' "$command_log" | awk '{ count = 0; for (i = 1; i <= NF; ++i) if ($i == "--needed") ++count; print count }')
    if [ "$needed_count" -ne 1 ]; then
        echo "makepkg command did not contain exactly one --needed" >&2
        cat "$command_log" >&2
        exit 1
    fi
}
...
assert_only_sudo_command "sudo pacman -Syu --needed"
assert_command "git clone https://aur.archlinux.org/clean-root.git clean-root"
assert_command "makepkg -sic --needed"
assert_single_makepkg_needed
...
assert_command "makepkg -sic --noconfirm --needed"
assert_single_makepkg_needed
assert_no_sudo_commands
assert_normal_request_log_nonempty
...
assert_only_sudo_command "sudo pacman -S --needed official-a"
assert_command "git clone https://aur.archlinux.org/clean-root.git clean-root"
assert_command "makepkg -sic --needed"
assert_single_makepkg_needed
...
assert_only_sudo_command "sudo pacman -S official-a --needed"
assert_command "makepkg -sic --needed"
assert_single_makepkg_needed
assert_normal_request_log_nonempty
...
run_aur_combination() {
    ...
    assert_command "$expected_makepkg"
    assert_single_makepkg_needed
    assert_command "git clone https://aur.archlinux.org/clean-root.git clean-root"
    assert_normal_request_log_nonempty
}

run_aur_combination rebuild "makepkg -sic -f --needed" --needed --rebuild
run_aur_combination cleanbuild "makepkg -sic -C --needed" --needed --cleanbuild
run_aur_combination rmdeps "makepkg -sic -r --needed" --needed --rmdeps
run_aur_combination noconfirm "makepkg -sic --noconfirm --needed" --needed --noconfirm
run_aur_combination all "makepkg -sic --noconfirm -f -C -r --needed" \
    --needed --rebuild --cleanbuild --rmdeps --noconfirm
