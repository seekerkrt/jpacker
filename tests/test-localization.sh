...
assert_english_messages() {
    ...
    assert_line 'reviewed_update_state=Reviewed state for PackageBase localized-base was published at generation 41; build and install outcomes are reported separately.' "$output_file"
...
assert_line 'reviewed_update_state=確認済み状態（PackageBase localized-base）を世代 41として公開しました。ビルドとインストールの結果は別に報告します。' "$ja_output"
