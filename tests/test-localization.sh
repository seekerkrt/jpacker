...
assert_english_messages() {
    ...
    assert_line 'reviewed_uncertain_failure=Reviewed source state publication outcome is uncertain; the build was not started and automatic retry is forbidden.' "$output_file"
...
assert_line 'reviewed_uncertain_failure=確認済みソースの状態公開結果を確定できません。ビルドは開始せず、自動再試行もしません。' "$ja_output"
