...
### 15. local PKGBUILDは`build --local`で明示し、remote package identityと分離する

#271では、既存の`build <pkg> [V=K...]`をremote package nameからbuild / installする入口として維持し、local PKGBUILDの正式入口を`moguet build --local <directory> [V=K...]`とする。`--local`は`build`専用のsource selectorであり、directory operandはexactly oneとする。`-Bi`や`build-local`のaliasは追加しない。directoryは1つのlocal PackageBase sourceを選び、採用したmetadata snapshotに宣言されたvalidかつuniqueな全`pkgname` childをfirst-seen orderのrequired root targetとする。

local rootはremote repository / AUR rootと異なるtyped identityとして、descriptor-firstで開いたdirectory、descriptorから導出したcanonical display path、retained filesystem identity、root `PKGBUILD` content snapshot、metadata provenanceを保持する。Git repositoryであることを要求せず、local rootへclone、fetch、reset、clean、merge、overwrite、deleteを行わない。root directory、root `PKGBUILD`、任意の`.SRCINFO`はeffective user所有かつgroup / other write不可とし、sticky bitを例外にしない。fileはdirectory descriptor相対でregular non-symlinkとして検証し、source snapshot確定前とexternal mutation前にidentityとcontent provenanceを再検証する。既存cache checkout lifecycleや、artifactだけを分離するfresh `PKGDEST`をuser-owned treeへ直接適用せず、buildはlocal treeを変更しない一貫したinvocation-owned source snapshotが完成してから接続する。

既存の安全な`.SRCINFO`はread-only metadataの第一候補とし、PackageBase、ordered children、version、architecture、architecture-qualified field、dependency、provides、conflicts、replacesをchild scope、qualifier、constraintを失わず保持する。metadata stateはmissing、unsafe、invalid、usable-unverified、known-staleを区別する。`PKGBUILD`のmtimeが`.SRCINFO`より新しい場合はknown-staleとするが、逆順をfreshnessの証明にはしない。one-off `V=K`を指定した場合は既存`.SRCINFO`がそのeffective environmentを表すと証明できないためknown-staleとして評価を要求し、同じordered environment snapshotを`makepkg --printsrcinfo`、`makepkg --packagelist`、buildへ渡す。unsafe filesystem stateはhard failureとし、missing / invalid / known-staleだけがPKGBUILD review後の明示的なno-default consentを経て`makepkg --printsrcinfo`評価へ進める。`--noedit`は評価許可ではなく、`--noconfirm`、non-TTY、cancel、EOFは評価を自動承認しない。生成metadataはinvocation内だけで使い、user-owned `.SRCINFO`を書き換えない。`deps` / `plan`相当のread-only境界はPKGBUILDを評価しない。

local root path、PackageBase、child nameをAUR root queryへ渡さず、local metadata failureをAUR absence、RPC failure、empty dependencyへflattenしない。local PackageBase、children、provides、version constraint、self-returnを含むback-edgeをtyped identityのままremote candidate queryより前にSlice 3へ渡し、constraint適合を確認する前にsame-PackageBase edgeをsatisfiedへ丸めたりcycle情報を捨てたりしない。Slice 3がeffective architecture、version / provides constraint、required child集合からinternal edge、未解決dependency、真のcycleを分類し、未解決dependencyだけを既存のrepo exact / AUR exact / provider policyへprojectする。ambiguous providerは#272のinvocation-local selection contractへ渡す。artifact build後は#268のordered required-child selection、identity / freshness / containment validation、install-reason policyを再利用し、local root childrenをExplicit、dependenciesをDependencyとして扱い、existing Explicitを降格しない。

このdecisionは#271のproduction contractを固定するが、現在のCLIへ`--local`を実装済みとするものではない。LocalSourceRootとread-only metadata、dependency plan projection、user-owned treeを変更しないbuild workspace、artifact / install execution、help / man / completion / localizationを後続sliceで揃え、不完全なlocal-build routeを有効化しない。
...
### 15. Local PKGBUILDs use explicit `build --local` routing and remain separate from remote package identities

#271 keeps the existing `build <pkg> [V=K...]` entry point for building and installing from a remote package name and adopts `moguet build --local <directory> [V=K...]` as the sole local-PKGBUILD entry point. `--local` is a source selector owned only by `build`, and it accepts exactly one directory operand. Moguet adds neither a `-Bi` nor a `build-local` alias. The directory selects one local PackageBase source, and every valid, unique `pkgname` child declared by the accepted metadata snapshot becomes a required root target in first-seen order.

A local root has a typed identity distinct from repository and AUR roots. It retains a descriptor-first opened directory, canonical display path derived from that descriptor, filesystem identity, root `PKGBUILD` content snapshot, and metadata provenance. It does not require a Git repository, and Moguet never clones, fetches, resets, cleans, merges, overwrites, or deletes the local root. The root directory, root `PKGBUILD`, and optional `.SRCINFO` must be owned by the effective user and must not be group- or other-writable; a sticky bit is not an exception. The files are validated relative to the directory descriptor as regular non-symlinks, and their identities and content provenance are revalidated before fixing the source snapshot and before external mutation. The cache-checkout lifecycle and a fresh `PKGDEST`, which isolates artifacts only, are not applied directly to a user-owned tree; execution is connected only after a consistent invocation-owned source snapshot can build without modifying that tree.

An existing safe `.SRCINFO` is the first read-only metadata candidate. It retains the PackageBase, ordered children, version, architecture, architecture-qualified fields, dependencies, provides, conflicts, and replaces without losing child scope, qualifiers, or constraints. Metadata states distinguish missing, unsafe, invalid, usable-but-unverified, and known-stale. A `PKGBUILD` mtime newer than `.SRCINFO` proves known staleness, but the reverse ordering does not prove freshness. When one-off `V=K` assignments are present, the existing `.SRCINFO` cannot prove metadata for that effective environment and is therefore known-stale; the same ordered environment snapshot is passed to `makepkg --printsrcinfo`, `makepkg --packagelist`, and the build. Unsafe filesystem state is a hard failure. Only missing, invalid, or known-stale metadata may proceed to `makepkg --printsrcinfo` after PKGBUILD review and explicit no-default consent. `--noedit` is not evaluation consent, while `--noconfirm`, non-TTY input, cancellation, and EOF never authorize evaluation. Generated metadata remains invocation-local and never rewrites the user's `.SRCINFO`. Read-only `deps` or `plan`-equivalent boundaries never evaluate a PKGBUILD.

The local root path, PackageBase, and child names never enter an AUR root query, and local metadata failure is not flattened into AUR absence, RPC failure, or empty dependencies. The local PackageBase, children, provides, version constraints, and back-edge information including self-returns reach Slice 3 as typed identities before any remote-candidate query. A same-PackageBase edge is not flattened to satisfied before constraint matching, and cycle information is not discarded. Slice 3 owns the classification of internal edges, unresolved dependencies, and real cycles from the effective architecture, version/provides constraints, and required-child set. Only unresolved dependencies are then projected into the existing repository-exact, AUR-exact, and provider policy; ambiguous providers use the invocation-local selection contract from #272. After artifact creation, the route reuses #268's ordered required-child selection, identity/freshness/containment validation, and install-reason policy. Local root children are Explicit, dependencies are Dependency, and an existing Explicit package is never demoted.

This decision fixes the production contract for #271 but does not claim that `--local` is implemented in the current CLI. Later slices must complete LocalSourceRoot and read-only metadata, dependency-plan projection, a build workspace that does not modify the user-owned tree, artifact and install execution, and help/man/completion/localization before enabling an incomplete local-build route.
