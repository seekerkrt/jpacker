...
このdecisionは#271のproduction contractを固定する。#271 Slice 5でLocalSourceRoot、read-only / evaluated metadata、dependency plan projection、user-owned treeを変更しないbuild workspace、artifact / install execution、help / man / completion / localizationを揃え、production CLIの`build --local`入口へ接続した。
...
This decision fixes the production contract for #271. Slice 5 of #271 completed LocalSourceRoot, read-only and evaluated metadata, dependency-plan projection, a build workspace that does not modify the user-owned tree, artifact and install execution, help, man pages, completion, and localization, and connected the `build --local` entry point to the production CLI.
